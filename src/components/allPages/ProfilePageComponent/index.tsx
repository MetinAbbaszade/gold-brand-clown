...
const ProfilePageComponent = () => {
	...
	const tabPanels: Record<typeof activeTab, React.ReactNode> = {
		Profile: <ProfileTab />,
		Orders: <OrdersTab />,
		...
		<Stack alignItems="center" justifyContent="center" width="100%">
			...
			>
				...
				<Stack
					sx={{
						background:
							"linear-gradient(to bottom, rgba(212,175,55,0.1), rgba(255,255,255,0))",
				...
				<Stack direction="row" width="100%" bgcolor="#f5f5f5">
					{(["Profile", "Orders", "Wishlist", "Settings"] as const).map(
						(label) => (
							<Button
								key={label}
								sx={{
									flex: 1,
									minWidth: 0,
									padding: { xs: "8px 4px", sm: "10px 0" },
									fontSize: { xs: "0.7rem", sm: "0.875rem" },
									borderBottom:
										activeTab === label ? "2px solid #d4af36" : "none",
									color: activeTab === label ? "#d4af36" : "#7a7a7a",
									"&:hover":
										activeTab === label
											? { bgcolor: "inherit" }
											: { color: "#9e7c0c", bgcolor: "inherit" },
								}}
				...
				{/* Tab content */}
				<Box sx={{ m: { xs: 2, sm: 3, md: 5 } }}>{tabPanels[activeTab]}</Box>
			</Stack>
		</Stack>
	);
};
